    ...
    useEffect(() => {
        ...
        const startScanner = async () => {
            try {
                ...
                // Use back camera if there are two cameras, otherwise use the only available camera
                const backCamera = devices.find(device =>
                    device.label.toLowerCase().includes('back') || device.label.toLowerCase().includes('rear')
                );

                const selectedDevice = devices.length === 1
                    ? devices[0]?.deviceId // Use front if only one camera
                    : backCamera?.deviceId || devices[0]?.deviceId; // Use back if available, otherwise fallback to first camera
                    ...
                    <div>
                        ...
                        <div className={NewProductscss.NewProductCamera}>
                            ...
                            <video
                                ref={videoRef}
                                style={{
                                    display: isProcessing ? 'none' : 'block', opacity: videoFade ? 1 : 0, transition: 'opacity 1s ease-in-out'
                                }}
                            />
                        </div>
                    </div>
